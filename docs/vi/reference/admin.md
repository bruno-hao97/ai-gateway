---
title: Admin (chỉ server)
description: Số dư merchant, gửi credits, đăng ký user
---

# Admin (chỉ server)

::: danger
Route `/admin/*` dùng **`x-admin-key`**, không phải Bearer token user.
**`GOMMO_ACCESS_TOKEN`** (merchant) chỉ nằm trong `.env` server.
:::

| Thao tác | Gommo upstream | AI Gateway |
|-----------|----------------|------------|
...
| `GOMMO_API_DOMAIN` | Domain merchant (mặc định `79ai.net`) |
| `GOMMO_MANAGER_ID` | `manager_id` cho register |
| `TOPUP_MERCHANT_BUFFER_CREDITS` | Buffer trước khi gửi (mặc định 300000) |
| `ADMIN_API_KEY` | Bảo vệ `/admin/*` |

Quy tắc Gommo: sau `sendBalances`, merchant phải còn **> 500.000** credits.
...
Quyền `sendBalances` do Gommo cấp cho **tài khoản merchant** domain.
