            ...
            <div v-if="memoryOpen" class="or-chat-composer-pop or-chat-composer-pop-wide" @click.stop>
              ...
              <label v-if="!streamForced" class="or-chat-tool-row">
                <span>
                  ...
                  <small>{{
                    isVi
                      ? 'Bật: gõ dần. Tắt: một cục JSON (model agent thường).'
                      : 'On: typewriter. Off: one-shot JSON (typical agent models).'
                  }}</small>
                </span>
                <input
                  type="checkbox"
